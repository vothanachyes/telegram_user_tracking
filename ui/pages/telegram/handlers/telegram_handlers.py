...
from ui.pages.telegram.handlers.import_users_handler import ImportUsersHandler
...
class TelegramHandlers:
    ...
    def __init__(
        ...
        self.import_users_handler = ImportUsersHandler(page, view_model)
    ...
    def handle_export_messages_excel(self):
        ...
        messages = self.view_model.get_messages(
            ...
            message_type_filter=self.messages_tab.filters_bar.get_message_type_filter()
        )
        if not messages:
    ...
    def handle_import_users(self):
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.debug("handle_import_users called in TelegramHandlers")
        
        if not self.page:
            logger.error("Page not set in TelegramHandlers")
            return
        
        group_id = self.users_tab.get_selected_group()
        group_name = None
        
        logger.debug(f"Selected group_id: {group_id}")
        ...
        # Update page reference in handler if needed
        if self.import_users_handler.page != self.page:
            self.import_users_handler.page = self.page
        
        self.import_users_handler.handle_import_users(group_id, group_name)
