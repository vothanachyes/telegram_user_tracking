"""
Data generator UI component for test data generation.
"""
...
class DataGeneratorApp:
    ...
    def __init__(self, page: ft.Page, db_path: Optional[str] = None, build_ui: bool = True):
        """
        Initialize the application.
        
        Args:
            page: Flet page instance
            db_path: Optional database path for dumping data
            build_ui: If True, builds UI and adds to page. If False, only initializes components.
        """
        self.page = page
        if build_ui:
        ...
        self.output_db = ft.Checkbox(label="Direct Database Dump", value=True)  # Default to True when opened from app
        self.db_path_input = ft.TextField(label="Database Path", value=db_path or "./data/app.db", width=300, visible=False)  # Hide since we use provided path
    ...
    def _on_generate_click(self, e):
        ...
        try:
            ...
            if self.output_db.value:
                ...
                # After successful dump, refresh the app if needed
                # The dialog will remain open so user can generate more data if needed
            else:
                # If not dumping to DB, just show success
    ...
    def build_content(self) -> ft.Container:
        """
        Build and return the content container (for use in dialogs).
        This allows the data generator to be embedded in a dialog.
        """
        # Date range section
        date_section = ft.Container(
            content=ft.Column([
