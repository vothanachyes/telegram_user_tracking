...
class FirebaseConfig:
    ...
    def get_notifications(self, user_id: Optional[str] = None, id_token: Optional[str] = None) -> List[dict]:
        ...
        try:
            ...
            url = f"{FIRESTORE_REST_URL}/projects/{self.project_id}/databases/(default)/documents/{FIRESTORE_NOTIFICATIONS_COLLECTION}"
            ...
            if response.status_code == 200:
                ...
                for doc in documents:
                    ...
                    # Convert Firestore document format
                    notification_data = self._convert_firestore_document(doc)
    ...
    def get_user_notification_status(self, user_id: str, notification_id: str, id_token: Optional[str] = None) -> Optional[dict]:
        ...
        try:
            ...
            url = f"{FIRESTORE_REST_URL}/projects/{self.project_id}/databases/(default)/documents/{FIRESTORE_USER_NOTIFICATIONS_COLLECTION}/{user_id}/notifications/{notification_id}"
            ...
            if response.status_code == 200:
                data = response.json()
                status_data = self._convert_firestore_document(data)
                if status_data:
    ...
    def mark_notification_read(self, user_id: str, notification_id: str, id_token: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Prepare document data in Firestore format
    ...
    def get_user_notification_statuses(self, user_id: str, id_token: Optional[str] = None) -> dict:
        ...
        try:
            ...
            url = f"{FIRESTORE_REST_URL}/projects/{self.project_id}/databases/(default)/documents/{FIRESTORE_USER_NOTIFICATIONS_COLLECTION}/{user_id}/notifications"
            ...
            if response.status_code == 200:
                ...
                documents = data.get("documents", [])
                for doc in documents:
                    # Extract notification_id from document name
                    doc_name = doc.get("name", "")
                    notification_id = doc_name.split("/")[-1] if "/" in doc_name else ""
                    
                    # Convert Firestore document format
                    status_data = self._convert_firestore_document(doc)
                    if status_data:
